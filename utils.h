...
/*
 * Connect a set of sockets given the addrinfo list, one per address.
 * Return the actual number of sockets opened in nr_fds.
 *
 * Note that if the function is unable to connect an address, it just
 * goes on.  It returns NULL if it is unable to connect any addresses.
 * Also, open IPV6 addresses first.  This way, addresses in shared
 * namespaces (like IPV4 and IPV6 on INADDR6_ANY) will work properly
 */
int *connect_socket(struct addrinfo *ai, unsigned int *nr_fds);
